...
class Blogger:
    ...
    def trading_depo_summary_message(
        ...
        if self.__blog_status:
            self.__send_text_message(
                f"Итоги по размеру депо за торговую сессию. "
    ...
    def fail_message(self):
        if self.__blog_status:
            self.__send_text_message(
                f"Во время торговли что-то пошло не так. Пробую закрыть позиции. "
    ...
    def summary_open_signal_message(self, trade_order: TradeOrder, open_order_state: OrderState):
        if self.__blog_status:
            ...
            self.__send_text_message(
                f"Открытый {signal_type} сигнал по {self.__trade_strategies[trade_order.signal.figi].ticker}. "
    ...
    def summary_closed_signal_message(self,
        ...
        if self.__blog_status:
            ...
            self.__send_text_message(
                f"Отработанный {signal_type} сигнал по {self.__trade_strategies[trade_order.signal.figi].ticker}. "
